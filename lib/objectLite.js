 ...
 *  @description  persistent object storage with sqlite
 *
...
*
* objectLite.run() fail object === Error()
* .onError set:
* 	this.lastError:null,
* 	this.lastErrorCode:null,
* 	this.lastErrorNo:null,
* 	this.lastErrorLoc:'',			//function, line, and col generating error
 */
...
var objectLite = {
	...
	/**
	 * Execute SQLite Select command and return all matching rows
...
function filterWhere(filter){
	...
	//log('fiilterWhere', filter)
	if(filter == null) return ''
	if(filter.constructor !== Array) filter = [filter]

	let result = ''
	for(let fltObj of filter){
		let clause = ''
...
function filterLimit(filter){
	//not used
	//assume: filter.constructor === Array
	let result = '', clause = ''
	for(let fltObj of filter){
		for(let key in fltObj){
			key = key.toLowerCase()
			if(key==='limit')
				clause += ' LIMIT '+fltObj[key]
			else
			if(key==='offset')
				clause += ' OFFSET '+fltObj[key]
		}
	}
