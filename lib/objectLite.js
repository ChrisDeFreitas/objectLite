...
objectLite.init = function(options){
	...
	try{
		...
		db.on('close', error => {
			if(error==null){ 
				if(debug) console.error('objectLite.SQLite.close() event: no errors')
			}
			else
				throw 'objectLite.close() SQLite error: '+error
		})
		db.on('error', error => {
...
//row methods
objectLite.cnt = async function(name, filter){
	...
	return await this.all(sql, null, function(rows){
		return rows[0].found
	})
}
...
function objFilter(obj, filterObj){
	...
	//return true/false
	//Assume: AND operations are specified within one object
	//				OR operations use seperate objects
